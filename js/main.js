...
const checkDomainUrl = (str) => {
  let regDomain =
    /^(http:|https:)\/\/?[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\(\)\*\+,;=.]+$/gm;
  return regDomain.test(str);
};

console.log;
checkDomainUrl("http://external.asd1230-123.asd_internal.asd.gm-_ail.com"); // true
...
const createLinksFromDomains = (str) => {
  // let regLink =
  // /^(http:|https:)\/\/?[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\(\)\*\+,;=.]+$/gi;
  let regLink =
    /(http:|https:)\/\/(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}/gi;
  let res = "";
  let str1;
  str.match(regLink).forEach(function (item) {
